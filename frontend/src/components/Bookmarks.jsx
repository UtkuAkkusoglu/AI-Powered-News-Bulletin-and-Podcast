...
function Bookmarks() {
  ...
  const fetchBookmarks = async () => {
    setLoading(true);
    try {
      const res = await fetchWithAuth(`${import.meta.env.VITE_API_URL}/bookmarks/?page=${page}&size=${pageSize}`);
      if (res.ok) {
  ...
  const handleRemove = async (newsId, e) => {
    e.stopPropagation();
    try {
      const res = await fetchWithAuth(`${import.meta.env.VITE_API_URL}/bookmarks/${newsId}`, { method: 'DELETE' });
      if (res.ok) {
        setBookmarks(prev => prev.filter(b => b.news_id !== newsId));
        setTotalCount(p => p - 1);
        showToast("Kaydedilenlerden kaldırıldı.", "success");
      }
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={{ padding: isMobile ? '5rem 0 2.5rem' : '2rem 0 2.5rem' }}>
        <h1 style={{ fontSize: isMobile ? '2rem' : '3rem', fontWeight: '900', margin: 0, color: 'white' }}>Kaydedilenler</h1>
        <p style={{ color: '#94a3b8', fontSize: '1.1rem', marginTop: '10px' }}>
          {totalCount > 0 ? `${totalCount} kayıtlı haber` : 'Henüz kayıtlı haber yok.'}
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', marginTop: '8rem', color: '#475569' }}>
          ...
          <p style={{ fontSize: '1.2rem' }}>Henüz haber kaydetmediniz.</p>
          <p style={{ fontSize: '0.9rem', marginTop: '8px' }}>Haber detayında "Kaydet" butonuna basarak buraya ekleyebilirsiniz.</p>
        </div>
      ) : (
        <>
