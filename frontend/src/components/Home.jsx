...
function Home() {
  ...
  // İlk yüklemede kullanıcı ilgi alanlarını çek
  useEffect(() => {
  ...
  // Filtreler veya sayfa değiştikçe haberleri çek
  useEffect(() => {
  ...
  const fetchNews = async (query = '', p = page, size = pageSize, catId = activeCategoryId) => {
    ...
    try {
      ...
      if (catId === 0) {
        // "Tümü" seçiliyse category_id gönderme
      } else if (catId) {
        params.set('category_id', catId);
      } else {
        // "İlgi Alanlarım" (null) seçiliyse interests_only gönder
  ...
  const styles = {
    container: { color: '#f1f5f9', fontFamily: "'Inter', sans-serif" },
    toast: {
    ...
    bentoGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '28px', marginBottom: '4rem' },
    newsCard: {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={{ padding: '2rem 1rem 2.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', maxWidth: '1400px', margin: '0 auto' }}>
        <div>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 1rem' }}>
        ...
        {loading ? (
          ...
          <>
            ...
            {/* --- GÜNCELLENEN PAGING ARAYÜZÜ (GERİ GELDİ) --- */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '3rem 0 10rem', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ marginTop: '4rem', background: 'rgba(30, 41, 59, 0.5)', padding: '2.5rem', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              {podcastStatus === 'ready' && podcastId && (
                <div style={{ textAlign: 'center' }}>
                  ...
                  <audio ref={audioRef} key={podcastId} controls preload="auto" src={`${import.meta.env.VITE_API_URL}/podcast/${podcastId}/audio?t=${Date.now()}`} style={{ width: '100%', borderRadius: '16px', filter: 'invert(10%) hue-rotate(180deg)' }}>Tarayıcınız ses çalmayı desteklemiyor.</audio>
                </div>
              )}
            </div>
          </div>
