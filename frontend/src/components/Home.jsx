...
function Home() {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      clearInterval(poll);
      setRefreshing(false);
      showToast("Gündem güncellendi!", "success");
    }
  };
  ...
  const styles = {
    mainContainer: { color: '#f1f5f9', fontFamily: "'Inter', sans-serif" },
    toast: {
    ...
    bentoGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '28px', padding: '0 1rem 4rem' },
    newsCard: {
    ...
    floatingPlayer: {
      ...
      width: '600px', background: 'rgba(15, 23, 42, 0.95)', backdropFilter: 'blur(24px)',
      border: '1px solid rgba(99, 102, 241, 0.3)', borderRadius: '24px',
      padding: '1.2rem 2rem', zIndex: 2000, boxShadow: '0 20px 50px rgba(0,0,0,0.6)',
  ...
  return (
    <div style={styles.mainContainer}>
      ...
      <div style={{ padding: '2rem 1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', maxWidth: '1400px', margin: '0 auto' }}>
        <div>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', padding: '0 1rem', marginBottom: '3rem' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(2, 6, 23, 0.85)', backdropFilter: 'blur(16px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1100 }}>
          <div style={{ background: 'rgba(15, 23, 42, 0.98)', border: '1px solid rgba(255,255,255,0.1)', padding: '4rem', borderRadius: '40px', maxWidth: '900px', width: '90%', maxHeight: '85vh', overflowY: 'auto', position: 'relative' }}>
            ...
            <div style={{ marginTop: '2rem' }}>
              ...
              {podcastStatus === 'processing' && <p style={{ color: '#818cf8' }}>🎧 Seslendiriliyor...</p>}
            </div>
          </div>
        </div>
      )}

      {/* 🔥 CİHAN'IN İSTEDİĞİ YÜZER MEDYA OYNATICI */}
        ...
        <div style={styles.floatingPlayer}>
          <div style={{ flex: 1 }}>
            <div style={{ color: 'white', fontSize: '0.9rem', fontWeight: '700', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {selectedNews?.title || "Podcast Oynatılıyor"}
            </div>
            <div style={{ color: '#818cf8', fontSize: '0.75rem', fontWeight: '800' }}>NEWSFLOW MEDYA</div>
          </div>
          <audio 
            ref={audioRef} autoPlay controls
            src={`${import.meta.env.VITE_API_URL}/podcast/${podcastId}/audio?t=${Date.now()}`}
            style={{ height: '35px', filter: 'invert(10%) hue-rotate(180deg)' }}
          />
          <button onClick={() => { setPodcastId(null); setPodcastStatus('idle'); }} style={{ background: 'transparent', border: 'none', color: '#64748b', cursor: 'pointer', fontSize: '1.2rem' }}>✖</button>
        </div>
      )}

      {suggestion && (
        <div style={{ position: 'fixed', bottom: '120px', right: '40px', background: 'rgba(15, 23, 42, 0.8)', backdropFilter: 'blur(16px)', border: '1px solid #10b981', padding: '24px', borderRadius: '24px', zIndex: 1001 }}>
          <p style={{ color: 'white', marginBottom: '15px' }}>{suggestion.message}</p>
          <button onClick={handleAcceptSuggestion} style={{ padding: '8px 16px', background: '#10b981', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>Ekle</button>
        </div>
      )}
    </div>
  );
